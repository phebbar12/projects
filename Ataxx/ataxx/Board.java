...
class Board {
    ...
    /** Return the square col of linearized index SQ. */
    static char unindexC(int sq) {
        return (char) ('a' + ((sq - (EXTENDED_SIDE * 2 + 2)) % EXTENDED_SIDE));
    }

    /** Return the square row of linearized index SQ. */
    static char unindexR(int sq) {
        return (char) ('1' + ((sq - (EXTENDED_SIDE * 2 + 2)) / EXTENDED_SIDE));
    }
        ...
        for (int i = 0; i < _board.length; i++) {
            if (get(i) == who) {
                for (int row = 0; row < 5; row++) {
                    for (int j = neighbor(i, -2, -2 + row);
                         j <= neighbor(i, 2, -2 + row); j++) {
                        if (get(j) == EMPTY) {
                            return true;
                        }
                    }
        ...
        if (move.isPass()) {
            ...
            for (int dr = -1; dr <= 1; dr++) {
                for (int dc = -1; dc <= 1; dc++) {
                    int index = neighbor(move.toIndex(), dc, dr);
    ...
    /** Returns whether game is over on this board. */
    boolean gameOver() {
        return _gameOver;
    }
    /** Checks if game is over on this board. */
    void checkgameOver() {
    ...
    boolean legalBlock(char c, char r) {
        ...
        int C = i % EXTENDED_SIDE;
        int R = i / EXTENDED_SIDE;

        int reflC = EXTENDED_SIDE - C - 1;
        int reflR = EXTENDED_SIDE - R - 1;

        return (get(i) == EMPTY
                && get(reflC + reflR * EXTENDED_SIDE) == EMPTY
                && get(reflC + R * EXTENDED_SIDE) == EMPTY
                && get(C + reflR * EXTENDED_SIDE) == EMPTY);
    }
    ...
    void setBlock(char c, char r) {
        ...
        unrecordedSet(C + R * EXTENDED_SIDE, BLOCKED);

        if (C != reflC) {
            unrecordedSet(reflC + R * EXTENDED_SIDE, BLOCKED);

        }
        if (R != reflR) {
            unrecordedSet(C + reflR * EXTENDED_SIDE, BLOCKED);

        }
        if (R != reflR && C != reflC) {
            unrecordedSet(reflC + reflR * EXTENDED_SIDE, BLOCKED);
